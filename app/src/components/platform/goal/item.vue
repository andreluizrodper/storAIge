    ...
    <div class="p-2 md:p-4 md:pb-2">
      <div class="flex gap-2">
        ...
        <div class="flex flex-col justify-center">
          ...
          <router-link
            :to="{ name: 'goal_single', params: { goal_id: item.id } }"
          >
            <span class="text-xs underline">{{ createdAt }}</span>
          </router-link>
        </div>
      </div>
      <div class="py-2">
...
import { updateGoal } from "@/utils/goals";
...
export default {
  ...
  methods: {
    ...
    async doArchivePost() {
      await updateGoal({ id: this.item.id, data: { archived: true } });
    ...
    async toggleReaction(reaction) {
      ...
      await updateGoal({ id: this.item.id, data: { reactions } });
    },
  },
};
</script>
